...
Page({
  ...
  async loadBattles() {
    ...
    try {
      ...
      const data = await battleApi.getBattleList(params);
      const list = data.records || data.list || data || [];
  ...
  onTeamChange(e) {
    ...
    this.setData({ 'filters.teamId': team.id || '' });
  },

  onTypeChange(e) {
    const idx = Number(e.detail.value);
    const item = this.data.typeOptions[idx] || { value: '' };
    this.setData({ 'filters.incomeExpenseType': item.value });
  },
  ...
  goBattle(e) {
    ...
    if (item.gameId) {
      wx.navigateTo({ url: `/pages/game-detail/game-detail?id=${item.gameId}` });
    } else {
      wx.navigateTo({ url: `/pages/battle-detail/battle-detail?id=${item.id}` });
    }
  }
});
